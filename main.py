    ...
    try:
        with open(SETTINGS_FILE, "r", encoding="utf-8") as f:
            ...
            for cid_str, cfg in data.items():
                ...
                result[cid] = {
                    "zones": [
                        np.array(p, dtype=np.int32) for p in cfg.get("zones", [])
                    ],
...
def run_camera_setup() -> int | None:
    ...
    while not stop_event.is_set():
        ...
        cv.putText(
            frame,
            "Click button to add your first source",
            (180, 190),
            cv.FONT_HERSHEY_SIMPLEX,
            0.5,
            (150, 150, 150),
...
def fix_checkpoint(input_path, output_path):
    ...
    if isinstance(checkpoint, dict) and any(
        ...
        fixed = {"model": checkpoint, "epoch": -1, "source": "converted"}
...
def main():
    ...
    runtime_cameras = {
        k: (v.split("/")[-1] if v else f"Cam {k}") for k, v in CAMERAS.items() if v
    }
    camera_selector = CameraSelector(cameras=runtime_cameras, current_id=DEFAULT_CAM_ID)
    ...
    def on_camera_changed(cam_id):
        ...
        if old_cam_id is not None and old_cam_id != cam_id:
            CAMERA_SETTINGS[old_cam_id] = {
                "zones": [
                    np.array(p, dtype=np.int32, copy=True) for p in zone_drawer.polygons
                ],
        ...
        zone_drawer.polygons = [
            np.array(p, dtype=np.int32, copy=True) for p in cfg["zones"]
        ]
    ...
    video_out = cv.VideoWriter(
        "out.avi", cv.VideoWriter_fourcc(*"XVID"), fps, (scaled_w, scaled_h)
    )
        ...
        while True:
                        ...
                        try:
                            ...
                            if len(detections) > 0 and len(detections.confidence) > 0:
                                detection_info = {
                                    ...
                                    "class_name": coco_dict.get(
                                        int(detections.class_id[0]), "unknown"
                                    ),
            ...
            if add_camera_requested:
                ...
                if url and url.strip():
                    ...
                    CAMERAS[new_id] = url
                    runtime_cameras[new_id] = (
                        url.split("/")[-1] if "/" in url else f"File {new_id}"
                    )
